...
const PlaceScreen = ({route: {params: {id}}, navigation: {navigate}}) => {
    const {title, address, image, lat, lon} = useSelector(({places}) => places).find(v => v.id === id);

    return (
        <ScrollView>
            <View style={styles.screen}>
                <Image style={styles.fullImage} source={{
                    uri: image
                }}/>
